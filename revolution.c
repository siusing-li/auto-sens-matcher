...
int main()
{
    ...
    printf("sens: %.3_lf | yaw: %.3_lf\n", sensitivity, yaw);
    printf("increment: %.6_lf deg/count\n", increment);
    printf("total counts for 360 deg: %.10_lf\n", total_counts);
    ...
    /*  // deprecated method of finding divisor
        int primes[] = {2, 3, 5, 7, 11, 13};
        while (1)
        {
            if (step >= 150 && step <= 600) {
                break;
            }
            for (int i = (int) (sizeof(primes) / sizeof(primes[0])); i > 0; i--)
            {
                printf("prime %d -- ", primes[i - 1]);

                if (step % primes[i - 1] == 0)
                {
                    step = (int) (step / primes[i - 1]);
                    printf("new step %d\n", step);
                    break;
                }
            }
    ...
    printf("fractional steps needed %6_lf\n", total_counts_fractional);
